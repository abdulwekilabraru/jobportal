...
function Infinity() {
  ...
  // List of social media links and icons
  const socialLinks = [
    {
  ...
  // Duplicate the socialLinks array to create an infinite scroll effect
          ...
          {duplicatedLinks.map((link, index) => (
            <li key={index}>
              <a
                target="_blank"
                href={link.href}
                className={`border ${colors[index % colors.length]} text-white text-2xl sm:grid hidden place-content-center p-2 rounded-md`}
              >
                {link.icon}
              </a>
            </li>
